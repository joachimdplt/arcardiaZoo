...
export default function Dashboard() {
    ...
    const user = auth?.user || {}; // ✅ Sécuriser les accès à user

    // Vérifier les rôles de l'utilisateur connecté
    const isAdmin = user.role && user.role.label === 'Admin';
    const isEmployee = user.role && user.role.label === 'Employee';

    // Obtenir le rôle de l'utilisateur pour l'afficher dans le titre
    const roleLabel = user.role ? user.role.label : 'Aucun rôle';
