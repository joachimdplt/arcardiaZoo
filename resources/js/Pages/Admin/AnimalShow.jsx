...
export default function AnimalShow({ animal, lastVeterinaryReport }) {
    const { auth } = usePage().props;
    const [showFeedingForm, setShowFeedingForm] = useState(false);
    
    // Vérification correcte du rôle Employee
    const isEmployee = auth?.user?.role?.label === 'Employee';
    const isAdmin = auth?.user?.role?.label === 'Admin';

    console.log('Auth:', auth); // Pour debug
    console.log('Role:', auth?.user?.role); // Pour debug
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="bg-white shadow-md rounded p-6">
                ...
                {/* Bouton Nourrir l'animal - Seulement pour les employés */}
                {isEmployee && (
                    <div className="mt-4">
                        <button
                ...
                {/* Boutons d'accès aux alimentations et rapports vétérinaires */}
